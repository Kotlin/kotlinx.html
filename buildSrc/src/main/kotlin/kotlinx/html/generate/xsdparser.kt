...
fun fillRepository() {
  ...
  schema.elementDecls.values.forEach { elementDeclaration ->
    ...
    val excluded =
      globalSuggestedAttributes.get(name)?.filter { it.startsWith("-") }?.map { it.removePrefix("-") } ?: emptyList()
    ...
    if (type.isComplexType) {
      ...
      if (contentTerm != null) {
        ...
        if (contentTerm.isModelGroup) {
          directChildren.addAll(contentTerm.asModelGroup().children.map { it.term }.filter { it.isElementDecl }
            .map { it.asElementDecl().name })
        }
      }
